...
| `spec team watch` | Live workspace-wide SSE tail (`GET /api/me/prompt-stream`). **Verbose by default** &mdash; full assistant prose, not summaries. **Code blocks inside the prose collapse to `[code: lang ~N lines]`** so two teammates' threads stay scannable; pass `--show-tool-runs` to leave code intact and append the structured per-call tool list (`Read auth.py`, `Edit auth.py`, `Bash "pytest -q"` …) under each assistant body. Each header carries the role badge (USER / AI / ERROR), source adapter, branch, bundle, time; the chip row underneath shows `cwd ~/code/repo`, `touched billing.py, +2 more`, and a short `session` id **painted in a stable per-session color** so concurrent threads from different teammates stay visually disambiguated. Tool-only assistant turns (no prose at all, e.g. older clients) are filtered unless the auto-critic flags them. **Interactive commands**: `/help`, `/summarize 2h`, `/flag <id> <kind>`, `/focus <handle>`, `/mute <handle>`, `/replay 10m`, `/search <term>`, `/critic on|off`, `/status` &mdash; stream keeps scrolling while you type. Flags: `--no-verbose`, `--no-critic`, `--no-commands`, `--no-heartbeat`, `--show-tool-runs`, `--notify` (ring the bell + macOS banner on block-severity critic hits). |
