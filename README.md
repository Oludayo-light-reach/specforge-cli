...
| `spec team watch` | Live workspace-wide SSE tail (`GET /api/me/prompt-stream`). **Verbose by default** &mdash; full assistant bodies, not summaries. Each header carries the role badge (USER / AI / ERROR), source adapter, branch, bundle, time; the chip row underneath shows `cwd ~/code/repo`, `touched billing.py, +2 more`, and a short `session` id when the broadcaster ships them. Tool-only assistant turns are filtered unless the auto-critic flags them. **Interactive commands**: `/help`, `/summarize 2h`, `/flag <id> <kind>`, `/focus <handle>`, `/mute <handle>`, `/replay 10m`, `/search <term>`, `/critic on|off`, `/status` &mdash; stream keeps scrolling while you type. Flags: `--no-verbose`, `--no-critic`, `--no-commands`, `--no-heartbeat`, `--show-tool-runs`, `--notify` (ring the bell + macOS banner on block-severity critic hits). |
