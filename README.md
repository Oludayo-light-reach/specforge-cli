...
| `spec team watch` | Live workspace-wide SSE tail (`GET /api/me/prompt-stream`). **Default (non-compact) layout:** user bodies up to the schema wire cap; **assistant prose is a ~400-character digest** (merged `text` + `summary`); fenced code in assistant prose collapses to `[code: lang ~N lines]` unless you pass **`--show-tool-runs`**, which shows **full** assistant text, preserves code blocks, and lists structured tool calls. After each paired reply in digest mode, a **`● turn complete`** line suggests **`/turn`** / **`/full`** with the session chip so you know when to drill in; those commands **re-fetch full turns from Cloud** (see [docs/team-watch-slash-commands.md](docs/team-watch-slash-commands.md) for every slash command, aliases, and sample output). Headers: USER / AI / ERROR, source, branch, bundle, time; chip row: `cwd`, `touched`, color **`session`** id. Flags include `--compact`, `--no-verbose`, `--no-critic`, `--no-commands`, `--no-heartbeat`, `--show-tool-runs`, `--notify`. |
